...
class PublishPage extends StatefulWidget {
  // NOUVEAU : On ajoute un paramètre pour savoir où revenir
  final int returnIndex;
  
  // Par défaut, on met 0 (Carte) ou 1 (Feed) selon votre BottomNav. 
  // J'initialise à 0 ici, mais on le changera lors de l'appel.
...
class _PublishPageState extends State<PublishPage> {
  // ... (Le début de la classe reste identique : Services, Controllers, InitState, Camera...)
  ...
  // ... (Gardez les méthodes _initCamera, _toggleCamera, _takePicture, _startRecording, _stopRecording, _handleNewMedia, _pickFromGallery, _retake identiques) ...
  // Je ne les réécris pas pour ne pas surcharger, elles ne changent pas.
  ...
  Future<void> _publishStory() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text("Publication de la story...")),
    );
    ...
    if (success) {
      ...
      Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(
          // On utilise widget.returnIndex passé au constructeur
          builder: (context) => BottomNavigationBarExample(initialIndex: widget.returnIndex), 
        ),
        (route) => false,
      );
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Story publiée !"), backgroundColor: dGreen),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
  ...
  // Le reste du fichier (showTripSelectionModal, _publishToTrip, build...) reste identique
  // mais assurez-vous que _publishToTrip utilise aussi la redirection correcte si besoin.
  ...
  Future<void> _publishToTrip(int tripId) async {
    ...
    if (!mounted) return;
    if (success) {
      // Pour un post normal, on retourne généralement sur la page 4 (Carte/Profil) ou 1 (Feed)
      // Ici je laisse 4 comme avant, ou vous pouvez utiliser widget.returnIndex aussi.
      Navigator.of(context).pushAndRemoveUntil(
      ...
      children: [
          ...
          Positioned(
            ...
            child: Center(
              child: Container(
                ...
                child: const Text("Enregistrement...", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ),
            ),
          ),
        
        // SÉLECTEUR (MEMORY / STORY / POST)
        Positioned(
        ...
        Positioned(
          ...
          child: Row(
            ...
            children: [
              if (!_isRecording)
                FloatingActionButton(heroTag: "gallery", backgroundColor: Colors.grey[800], onPressed: _pickFromGallery, child: const Icon(Icons.photo_library, color: Colors.white))
              else const SizedBox(width: 56),

              GestureDetector(
                onLongPress: _startRecording, onLongPressUp: _stopRecording, onTap: _takePicture,
                child: Container(
                  width: 80, height: 80,
                  decoration: BoxDecoration(shape: BoxShape.circle, color: _isRecording ? Colors.red : Colors.white, border: Border.all(color: Colors.white, width: _isRecording ? 6 : 4)),
                  child: _isRecording ? const Icon(Icons.stop, color: Colors.white, size: 40) : null,
                ),
              ),

              if (!_isRecording)
                FloatingActionButton(heroTag: "switch", backgroundColor: Colors.grey[800], onPressed: _toggleCamera, child: const Icon(Icons.flip_camera_ios, color: Colors.white))
              else const SizedBox(width: 56),
            ],
          ),
        ),
  ...
  Widget _buildTypeSelector(String type) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Text(type, style: TextStyle(color: isSelected ? Colors.white : Colors.white70, fontWeight: FontWeight.bold, fontSize: 14, letterSpacing: 0.5)),
      ),
    );
  }

  Widget _buildPreviewView() {
      // ... (Code identique à la version précédente : PageView, TextField, Button Suivant...)
            ...
            children: [
              ...
              ClipRRect(
                ...
                child: BackdropFilter(
                  ...
                  child: Container(
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [_buildTypeSelector("POST"), _buildTypeSelector("MEMORY"), _buildTypeSelector("STORY")]),
                  ),
                ),
              ),
              ...
              // ... Location Button ...
               ElevatedButton.icon(
                ...
                label: Text(selectedLat == null ? "Ajouter une localisation" : "Localisation : ${selectedLat!.toStringAsFixed(4)}, ${selectedLon!.toStringAsFixed(4)}"),
                onPressed: () async {
              ...
              TextField(controller: _captionController, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(hintText: "Légende...", hintStyle: TextStyle(color: Colors.grey), border: InputBorder.none)),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  TextButton(onPressed: _retake, child: const Text("Annuler", style: TextStyle(color: Colors.white))),
                  ElevatedButton(
                    style: ElevatedButton.styleFrom(backgroundColor: dGreen, foregroundColor: Colors.black),
                    onPressed: _handlePublish,
                    child: Text(_postType == "STORY" ? "Publier ma Story" : "Suivant"),
                  ),
                ],
              ),
            ],
