...
class _ProfilePageState extends State<ProfilePage>
  ...
  void _navigateToChat() {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => ChatPage(conversation: conversation, onSend: (String conversationId, String text) {  },),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    String pseudo = _userData!['username'] ?? _userData!['pseudo'] ?? 'Utilisateur';
    String bio = _userData!['bio'] ?? '';
    
    // On autorise le null ici pour éviter les crashs si l'image manque
  ...
  Widget _buildProfileHeader(String? avatarUrl, String posts, String followers, String following) {
    return Padding(
      ...
      child: Row(
        children: <Widget>[
          Stack(
            children: <Widget>[
              // Si avatarUrl existe => image, sinon => icône
              CircleAvatar(
                radius: 40,
              ...
              if (isMyProfile)
                Positioned(
                  ...
                  child: Container(
                    ...
                    child: const Icon(Icons.add_circle, color: Colors.black, size: 24),
                  ),
                ),
            ],
          ),
        ...
        children: [
          Expanded(
            child: ElevatedButton(
              ...
              child: Text(
                _isFollowing ? 'Ne plus suivre' : (_followsMe ? 'Suivre en retour' : 'Suivre'),
    ...
    return GridView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        return FutureBuilder<List<dynamic>>(
          future: _postService.getMediaTripPosts(post['post_id']),
          builder: (context, snapshot) {
            ...
            return InkWell(
              onTap: () async {
                 // 1. Correction ID : on check 'id' et 'user_id'
                 final String myId = isMyProfile 
                    ? (_userData!['id']?.toString() ?? _userData!['user_id']?.toString() ?? "0") 
                    : "0";

                 // 2. Correction Refresh : On récupère le retour (true/false)
                ...
                // 3. Si on revient avec 'true', on recharge les données
  ...
  Widget _buildTripsGrid() {
    Widget content;
    if (_myTrips.isEmpty) {
      content = const Center(child: Text("Aucun voyage.", style: TextStyle(color: Colors.grey)));
    } else {
      content = Padding(
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            const Text("Impossible de charger le profil", style: TextStyle(color: Colors.white)),
...
// ------------------------------------------------------------------
// --- WIDGET VOYAGE (Avec Fallback Avion) ---
// ------------------------------------------------------------------
...
class _TripGridItemState extends State<TripGridItem> {
  ...
  // Fallback avec l'avion sur fond vert (dGreen)
