...
import 'package:spotshare/services/user_service.dart'; // <--- IMPORT AJOUTÉ ICI
...
class _PostCardState extends State<PostCard> with TickerProviderStateMixin {
  ...
  // Utility to expose to parent if needed
  void setFeedVisibility(bool visible) {
    // no-op here; kept for API symmetry if parent wants to call
  }
  ...
  Widget build(BuildContext context) {
    ...
    final bool hasStory =
        widget.storyGroup != null && widget.storyGroup!.stories.isNotEmpty;
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              Row(
                children: [
                  GestureDetector(onTap: _onAvatarTap, child: avatarWidget),
...
class _MediaPostItemState extends State<_MediaPostItem> {
  ...
  void initState() {
    ...
    // Pause videos automatically when the feed page is hidden
    _feedListener = () {
      try {
        if (!feedPageVisible.value && _isVideo && _isInitialized) {
          _videoController.pause();
          _videoController.seekTo(Duration.zero);
        }
      } catch (_) {}
    };
    feedPageVisible.addListener(_feedListener!);
  }
  ...
  @override
  void dispose() {
    if (_feedListener != null) feedPageVisible.removeListener(_feedListener!);
    if (_isVideo) {
...
class _CommentsSheetState extends State<_CommentsSheet> {
  ...
  Future<void> _loadCurrentUser() async {
    final profile =
        await getMyProfile(); // Maintenant cette méthode est reconnue
      ...
      children: [
        ...
        if (hasReplies && !isExpanded)
          _buildRepliesToggle(
            ...
            onTap: () {
              setState(
                () => _visibleRepliesCount[rootComment.id] = math.min(
                  totalReplies,
                  3,
                ),
              );
                    ...
                    children: [
                      ...
                      if (hasMore) ...[
                        GestureDetector(
                          onTap: () {
                            setState(
                              () => _visibleRepliesCount[rootComment.id] = math
                                  .min(totalReplies, showCount + 3),
                            );
                          },
                          child: Text(
